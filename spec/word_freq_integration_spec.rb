...
describe('The word_freq path', {:type => :feature}) do
  it('processes the user\'s word and sentence and returns "1 times" if the word occurs once') do
    visit('/')
    fill_in('word', :with => 'won\'t')
    fill_in('sentence', :with => 'Won\'t you ask the camel to stop spitting? He is getting spit on my wontons.')
    click_button('Search')
    expect(page).to have_content('Your word, "won\'t", was found 1 times')
  end
  ...
  #   click_button('Search')
  #   expect(page).to have_content('Your word, "won\'t", was found 2 times')
  # end
end
